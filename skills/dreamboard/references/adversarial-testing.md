...
```bash
bun apps/dreamboard-cli/src/skills/scripts/events-extract.ts --type ACTION_REJECTED --field message.reason
bun apps/dreamboard-cli/src/skills/scripts/events-extract.ts --type YOUR_TURN --field message.availableActions --limit 1
```
...
```bash
bun apps/dreamboard-cli/src/skills/scripts/events-extract.ts --type YOUR_TURN --player player-2 --field message.gameState.hands
```
