...
Current example workspace (`things-in-rings`) includes bases like `initial-turn` and `after-first-placement`, plus scenarios under `test/scenarios/`.
...
`generate` and `run` both accept `--env` and `--token`.
