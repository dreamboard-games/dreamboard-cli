...
10. Validate the live UI with `dreamboard run` and `dreamboard screenshot`
...
- Use `dreamboard screenshot` to verify initial state, active player, round resolution, and game-over feedback on realistic viewport sizes.
...
- `dreamboard pull` overwrites local files unless `--force` is provided.
