...
function parseTypedBaseDefinition(value: unknown): TypedBaseDefinition {
  ...
  return value as TypedBaseDefinition;
}
...
function toCardId(cardType: string, countIndex: number, count: number): string {
  return count === 1 ? cardType : `${cardType}-${countIndex + 1}`;
}

function expandCardSet(cardSet: CardSetDefinition): {
  cardIds: string[];
  cards: Record<string, Record<string, unknown>>;
} {
  if (cardSet.type !== "manual") {
    throw new Error(
      `Reducer-native test harness currently supports only manual card sets. Unsupported card set '${cardSet.id}' uses type '${cardSet.type}'.`,
    );
  }
  ...
  for (const card of cardSet.cards) {
    for (let index = 0; index < card.count; index += 1) {
      const cardId = toCardId(card.type, index, card.count);
      cardIds.push(cardId);
      cards[cardId] = {
        id: cardId,
...
function createInitialRawTable(
  manifest: GameTopologyManifest,
  playerIds: string[],
  random: KotlinSeededRandom,
): Record<string, unknown> {
  const cardSetCards = new Map<string, string[]>();
  const cards: Record<string, Record<string, unknown>> = {};

  for (const cardSet of manifest.cardSets) {
    const expanded = expandCardSet(cardSet);
    cardSetCards.set(cardSet.id, expanded.cardIds);
    Object.assign(cards, expanded.cards);
  }
  ...
  for (const zone of sharedZones) {
    ...
    const shuffled = shuffleWithKotlinRandom(
      (cardSetCards.get(primaryCardSetId) ?? []).filter(
  ...
  return {
    ...
    decks: sharedZones.map((zone) => zone.id),
    hands: playerZones.map((zone) => zone.id),
    publicHands: playerZones
...
class ShadowReducerRuntime {
  ...
  async start(): Promise<void> {
    ...
    const table = createInitialRawTable(this.manifest, this.playerIds, random);
...
export async function runReducerNativeScenarios(options: {
  ...
  const [bases, scenarios, generatedBaseStates, manifest, gameModule] =
    await Promise.all([
      loadTypedBases(options.projectRoot),
      loadTypedScenarios(options.projectRoot, {
        scenarioPath: options.scenarioPath,
