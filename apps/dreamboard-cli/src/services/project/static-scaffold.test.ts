...
test("scaffolds static framework files locally", async () => {
  ...
  try {
    ...
    expect(rootPkg.dependencies?.["framer-motion"]).toBeUndefined();
    expect(rootPkg.dependencies?.["lucide-react"]).toBeUndefined();
    expect(rootPkg.dependencies?.["@use-gesture/react"]).toBeUndefined();
    ...
    const uiPkg = JSON.parse(
      await Bun.file(path.join(tempRoot, "ui", "package.json")).text(),
    ) as {
      dependencies?: Record<string, string>;
    };
    expect(uiPkg.dependencies?.["react"]).toMatch(/^\^/);
    expect(uiPkg.dependencies?.["zod"]).toBeUndefined();
    expect(uiPkg.dependencies?.["framer-motion"]).toBeUndefined();
    expect(
      await Bun.file(
        path.join(
          tempRoot,
...
test("scaffolds local maintainer registry metadata into package.json and .npmrc", async () => {
  ...
  try {
    ...
    expect(rootPkg.devDependencies?.["@dreamboard/testing"]).toBe(
      "0.1.0-local.1",
    );
...
test("update removes the legacy framework version marker from package.json", async () => {
  const { scaffoldStaticWorkspace } = await loadStaticScaffold();
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "db-static-scaffold-"));
  const packageJsonPath = path.join(tempRoot, "package.json");
  ...
  try {
    ...
    packageJson.dreamboardFrameworkVersion = 2;
    await Bun.write(
      packageJsonPath,
      `${JSON.stringify(packageJson, null, 2)}\n`,
    );
    ...
    const refreshedPackageJson = JSON.parse(
      ...
      dreamboardFrameworkVersion?: number;
    };
    expect(refreshedPackageJson.dreamboardFrameworkVersion).toBeUndefined();
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
});
