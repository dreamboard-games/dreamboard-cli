...
async function writeInitialScenario(
  ...
  const content = `${GENERATED_SCENARIO_PREFIX}
...
`;
  ...
  if (
    ...
    isFrameworkOwnedScenarioSeed(existing)
  ) {
    await writeTextFile(filePath, content);
  }
}
...
function isFrameworkOwnedScenarioSeed(
  existingContent: string | null | undefined,
): boolean {
  if (existingContent == null) {
    return false;
  }
  return existingContent.startsWith(GENERATED_SCENARIO_PREFIX);
}
