...
| Type | Required fields | Notes |
| --- | --- | --- |
| `SquareSpaceSpec` | `id`, `row`, `col` | Optional `typeId`, `label`, `fields` |
| `SquareEdgeSpec` | `ref` | `ref.spaces` must resolve to exactly one shared border; optional `typeId`, `label`, `tags`, `fields` |
| `SquareVertexSpec` | `ref` | `ref.spaces` must resolve to exactly one shared corner; optional `typeId`, `label`, `tags`, `fields` |
