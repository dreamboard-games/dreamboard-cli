...
import ManifestCardSetsExample from '/snippets/examples/manifest-card-sets.mdx'
import ManifestSetupMetadataExample from '/snippets/examples/manifest-setup-metadata.mdx'
...
contents still belong in reducer setup code. In a hex trading game, the
...
```tsx
import { SlotSystem, useGameView, type SlotDefinition } from "@dreamboard/ui-sdk";
