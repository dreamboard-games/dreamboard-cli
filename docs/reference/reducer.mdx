...
| `SquareEdgeFieldsByBoardId` | Runtime square board ID -> per-edge `fields` shape |
| `SquareVertexFieldsByBoardId` | Runtime square board ID -> per-vertex `fields` shape |
| `TiledEdgeFieldsByBoardId` | Runtime tiled board ID -> per-edge `fields` shape |
| `TiledVertexFieldsByBoardId` | Runtime tiled board ID -> per-vertex `fields` shape |
...
| `boardLayoutById` | Runtime board ID -> `generic`, `hex`, or `square` |
...
| `edgeIdsByTypeId` | Tiled edge `typeId` -> edge IDs |
| `vertexIdsByTypeId` | Tiled vertex `typeId` -> vertex IDs |
