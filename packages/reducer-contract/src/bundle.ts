...
/**
 * Canonical erased GraalJS boundary for `createReducerBundle(...)`.
 *
 ...
 * authored bundle stays strongly typed in `@dreamboard/app-sdk/reducer`, but every
...
export interface ReducerBundleContract {
  ...
  initialize(input: Wire.InitializeRequest): MaybePromise<Wire.JsonValue>;
  initializePhase(
    input: Wire.InitializePhaseRequest,
  ): MaybePromise<Wire.JsonValue>;
  validateInput(
