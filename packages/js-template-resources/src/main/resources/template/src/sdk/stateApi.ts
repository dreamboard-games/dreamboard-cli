...
export interface TypedKv<TStore> {
  ...
  getOrThrow<K extends keyof TStore & string>(key: K): TStore[K];
...
function createDeckStateApi(state: GameState): DeckStateApi {
  return {
    getCards<D extends DeckId>(deckId: D): CardIdsByDeckId[D] {
      return Object.entries(state.componentLocations)
        ...
        .map(([cardId]) => cardId as CardId) as CardIdsByDeckId[D];
    },
